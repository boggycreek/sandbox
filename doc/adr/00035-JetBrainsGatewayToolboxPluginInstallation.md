...
## Decision
We introduce a first-class `sndbx plugin` command domain in the `sndbx` CLI and an accompanying JetBrains Gateway / Toolbox plugin architecture (`com.boggycreek.sndbx.gateway`):

### 1. `sndbx plugin <toolbox|vscode>` CLI Subcommand
Operators can install and configure IDE integrations directly via the CLI:
```bash
...
Rather than requiring a pre-installed JDK or Gradle build environment on the operator's machine, `sndbx plugin toolbox` dynamically generates and deploys a valid plugin JAR archive:
...
### 3. Transparent Host SSH Configuration Inclusion
JetBrains Gateway and Toolbox leverage OpenSSH host definitions from `~/.ssh/config`. `sndbx plugin toolbox` and `sndbx plugin vscode` inspect `~/.ssh/config` and guarantee the presence of:
```ssh
Include ~/.local/share/agent-sandbox/ssh_config
```
This ensures that all running agents (`sndbx-<name>`) immediately appear in JetBrains Gateway's SSH connection browser without restarting the container or manually managing ports and host keys.

### 4. Marketplace Independence
By distributing the plugin through `sndbx plugin toolbox`, development velocity is decoupled from JetBrains Marketplace publishing timelines. Once marketplace publishing is desired in a later release, the plugin can be submitted to plugins.jetbrains.com while retaining `sndbx plugin toolbox` as an offline and development fallback.
...
## Consequences
### Positive
- One-command onboarding for JetBrains Gateway and Toolbox users (`sndbx plugin toolbox`).
...
### Negative
- Local plugin directory deployment requires restarting active JetBrains IDE or Gateway instances to pick up newly added plugin jars.
